    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                ...
                ArrayList<String> dcontents = new ArrayList<String> ();

                while (matcher.find()) {
                    w.set(matcher.group());
                    String word = w.toString().toLowerCase();
                    dcontents.add(word);
                }

                for (int i = 0; i < dcontents.size(); i++) {
                    
                    double tempD1 = Double.POSITIVE_INFINITY;
                    double tempD2 = Double.POSITIVE_INFINITY;
                    double minDist = 0;

                    for (int j = i; j >= 0; j--) {
                        if (dcontents.get(j).equals(targetGram)) {
                            tempD1 = Math.min(tempD1, Math.abs(j - i));
                        }
                    }
                    for (int k = i; k < dcontents.size(); k++) {
                        if (dcontents.get(k).equals(targetGram)) {
                            tempD2 = Math.min(tempD2, Math.abs(k - i));
                        }
                    }
                    minDist = Math.min(tempD1, tempD2);

                    if (! dcontents.get(i).equals(targetGram)) {
                        context.write(new Text(dcontents.get(i)), new DoublePair(1.0, (func.f(minDist))));
                    }
                }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                if (sw > 0.0) {
                    rate = sw * Math.pow(Math.log(sw), 3.0) / (aw);
                } else {
                    rate = 0.0;
                }
            ...
            public void reduce(DoubleWritable key, Iterable<Text> values,
                ...
                for (Text val : values) {
                    if (n < N_TO_OUTPUT) {
                        ...
                        context.write(new DoubleWritable(key.get() * (-1)), val);
                        n += 1;
                    }
                }
            }
